...
export const SimulationShell = ({
  ...
  return (
    <SimulationDeepStudyProvider value={deepStudyContextValue}>
      <div className="flex min-h-[calc(100vh-9rem)] flex-col overflow-hidden rounded-[28px] border border-white/8 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.07),transparent_30%),linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] shadow-[0_24px_80px_rgba(2,8,23,0.26)] backdrop-blur-sm">
        <div className="border-b border-white/8 bg-black/10 px-4 py-3 md:px-5">
          <div className="flex min-w-0 items-start gap-3">
            <Button
              ...
              size="sm"
              onClick={onExit}
              className="mt-0.5 h-8 shrink-0 rounded-xl border-white/10 bg-white/[0.03] px-3 text-foreground hover:bg-white/[0.06] hover:text-foreground"
            >
            ...
            <div className="min-w-0 flex-1 space-y-1">
              ...
              <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1">
                ...
                <div className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.03] px-2.5 py-1 text-xs text-foreground/85">
                  <Sparkles className="h-3 w-3 text-primary/80" />
                  <span className="font-semibold">
                    {conceptSteps.length > 0 ? clampedStep + 1 : 0}/{conceptSteps.length}
                  </span>
